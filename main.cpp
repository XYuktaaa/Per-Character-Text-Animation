                    ...
                    int w, int h, gpointer)
{
    ...
    double t = std::chrono::duration<double>(now - g_start).count();

    render_frame(cr, g_glyphs, g_mode, t, g_stagger, g_speed, w, h, g_selected_glyph);
...
static void activate(GtkApplication* app, gpointer) {
    ...
    cairo_surface_t* tmp = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, 1, 1);
    cairo_t* tmp_cr = cairo_create(tmp);
    g_glyphs = extract_glyphs("SYNFIG", "Sans Bold 72", tmp_cr);
    cairo_destroy(tmp_cr); cairo_surface_destroy(tmp);
    ...
    GtkWidget* statusbar = gtk_label_new(
        "TextGroupLayer PoC v2  ·  PangoLayoutIter  ·  pango_cairo_show_glyph_string()  ·  GTK4");
